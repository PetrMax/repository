package model;

import javax.persistence.Entity;
import javax.persistence.ManyToOne;

import org.apache.openjpa.persistence.jdbc.ForeignKey;

@Entity
public class TableAnswer {
	@ForeignKey(columnNames="key")
	int key;	
	String answer_1;
	String answer_2;
	String answer_3;
	String answer_4;
	int true_answer_number;
	
	@ManyToOne
	TableQuestions questions;

	protected int getId() {
		return key;
	}

	protected void setId(int key) {
		this.key = key;
	}

	protected String getAnswer_1() {
		return answer_1;
	}

	protected void setAnswer_1(String answer_1) {
		this.answer_1 = answer_1;
	}

	protected String getAnswer_2() {
		return answer_2;
	}

	protected void setAnswer_2(String answer_2) {
		this.answer_2 = answer_2;
	}

	protected String getAnswer_3() {
		return answer_3;
	}

	protected void setAnswer_3(String answer_3) {
		this.answer_3 = answer_3;
	}

	protected String getAnswer_4() {
		return answer_4;
	}

	protected void setAnswer_4(String answer_4) {
		this.answer_4 = answer_4;
	}

	protected int getAnswer_true() {
		return true_answer_number;
	}

	protected void setAnswer_true(int answer_true) {
		this.true_answer_number = answer_true;
	}

	protected TableQuestions getQuestions() {
		return questions;
	}

	protected void setQuestions(TableQuestions questions) {
		this.questions = questions;
	}

	@Override
	public String toString() {
		return "TableAnswer [key=" + key + ", answer_1=" + answer_1
				+ ", answer_2=" + answer_2 + ", answer_3=" + answer_3
				+ ", answer_4=" + answer_4 + ", answer_true=" + true_answer_number
				+ ", questions=" + questions + "]";
	}

}
